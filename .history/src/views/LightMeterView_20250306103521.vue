...
function analyzeBrightness() {
  ...
  if (typeFlag.value === 'aperture') {
    ...
    const shutterValue = Math.pow(2, -(calculatedEV - exposureCompensation.value)) * 
      (100 / Number(iso.value)) * Math.pow(aperture.value, 2)
    ...
    // 快门优先模式：计算光圈值，加入曝光补偿
    const calculatedAperture = Math.sqrt(shutterSpeed.value * 
      (100 / Number(iso.value)) * Math.pow(2, calculatedEV - exposureCompensation.value))
      ...
      <div class="exposure-info">
        ...
        <div class="exposure-params">
          ...
          >
            ...
            <span class="value">{{ shutterSpeed === 0 ? '-' : (shutterSpeed >= 1 ? `${Math.round(shutterSpeed)}s` : `1/${Math.round(1 / shutterSpeed)}`) }}</span>
          </div>
          <div
            class="param"
